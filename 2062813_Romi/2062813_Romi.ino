...
void setup()
{
  ...
  pinMode(buttonPinB, INPUT);// for button B reading
  ...
  delay(3000);
  buttonNew_B = digitalRead(buttonPinB);
...
/*void FinalRot() { // Not needed,trying something out
  last_time=millis();
  if (buttonNew_B == 1) {
   float speed = 40.0f;
    unsigned long time_now = millis();
    unsigned long elapsed_time = time_now - last_time;
    if (elapsed_time < 700 ) {
      leftMotor(speed);
      rightMotor(-speed);
    }
    else if(elapsed_time >= 700) {
      state = 9;
    }
  }
  else
